	...
	forEachUnaryClient(t, func(t *testing.T, client *testutil.BAMLRestClient) {
		t.Run("two_client_rotation", func(t *testing.T) {
			...
			for i := 0; i < runs; i++ {
				...
				// RR is BuildRequest-only (skipIfNoBuildRequest gate at
				// the top of TestRoundRobinCall): the X-BAML-Path header
				// must always read "buildrequest" on a successful unary
				// RR response. Without this pin, a regression that
				// silently routes RR to the legacy path would still
		...
		t.Run("round_robin_metadata_headers", func(t *testing.T) {
			...
			// Identity cross-check: the response body carries the scenario
			// id of whichever child actually served this request. Assert
			// the Selected header names that same child. Without the
			// payload check, a metadata/dispatch desync — header reports
			// FallbackPrimary while BAML routed to FallbackSecondary —
			// would pass every other assertion (header present, index
			// in range) because hit-count or presence checks alone can't
			// bind metadata identity to a specific dispatch.
			selected := resp.Headers.Get(testutil.HeaderBAMLRoundRobinSelected)
			actualChild := clientForContent(string(resp.Body))
			if actualChild == "" {
				t.Errorf("response body %q doesn't identify a known child", string(resp.Body))
		...
		t.Run("three_client_rotation", func(t *testing.T) {
			...
			for i := 0; i < runs; i++ {
				...
				// Identity cross-check: the response body carries the
				// scenario id of whichever child actually served this
				// request. Assert the Selected header names that same
				// child. The header/index cross-check above proves
				// internal consistency between the two header fields,
				// but only the payload check binds the metadata
				// identity to the actual dispatch.
				actualChild := clientForContent(string(resp.Body))
				if actualChild == "" {
					t.Errorf("Call %d: response body %q doesn't identify a known child", i, string(resp.Body))
				} else if actualChild != selected {
					t.Errorf("Call %d: body was served by %q but Selected header says %q (metadata/dispatch desync)", i, actualChild, selected)
				}
			}
...
func TestRoundRobinStream(t *testing.T) {
	...
	t.Run("rotates_across_streams", func(t *testing.T) {
		...
		for i := 0; i < runs; i++ {
			...
			// Validated against wantChildren (a non-empty literal) so
			// the modulo divisor never goes to zero — equalSliceStrings
			// above only logs on mismatch, so a regression that ships
			// nil/empty Children would otherwise reach this division
			// and panic, masking the actual desync.
			if prevIndex >= 0 {
				wantIdx := (prevIndex + 1) % len(wantChildren)
				if tracker.planned.RoundRobin.Index != wantIdx {
					t.Errorf("Stream %d: expected Index=%d (prev=%d, step=+1 mod %d), got %d",
						i, wantIdx, prevIndex, len(wantChildren), tracker.planned.RoundRobin.Index)
				}
			}
...
func TestRoundRobinStreamWithRaw(t *testing.T) {
	...
	t.Run("raw_propagates_across_streams", func(t *testing.T) {
		...
		// runs=4 covers at least one wrap-around on a 2-child RR —
		// indices must step 0,1,0,1 or 1,0,1,0. runs=2 was too short
		// to catch a counter stuck at its initial offset.
		const runs = 4
		seenRaws := map[string]bool{}
		prevIndex := -1
		for i := 0; i < runs; i++ {
			...
			// counter by exactly one. A stuck or double-advanced
			// counter would manifest as a skipped or repeated index
			// here even when the distinct-payloads assertion below
			// still passed. Divisor is wantChildren (a non-empty
			// literal) so a regression that ships nil/empty Children
			// can't reach this division and panic — equalSliceStrings
			// above only logs on mismatch.
			if prevIndex >= 0 {
				wantIdx := (prevIndex + 1) % len(wantChildren)
				if tracker.planned.RoundRobin.Index != wantIdx {
