		...
		case event, ok := <-events:
			if !ok {
				if result.finalEvent == nil {
					t.Fatalf("stream closed without a terminal SSE event; raw snapshots: %d, reasoning snapshots: %d, data events: %d", len(result.rawSnapshots), len(result.reasoningSnapshots), len(result.dataEvents))
				}
				return result
			}
			if event.IsMetadata() {
