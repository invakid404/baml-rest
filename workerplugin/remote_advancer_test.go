...
func TestRemoteAdvancer_PropagatesParentCancellation(t *testing.T) {
	// Regression for CR-9: the RPC must inherit cancellation from the
	// per-request parent context rather than always using Background.
	// Previously a disconnected caller would leave the FetchAdd running
	// for up to r.timeout (5s) before the deadline fired.
