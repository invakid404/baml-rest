...
//
// onAttach is set to a no-op rather than left nil so the only
// misconfigured dependency under test is the broker. Without this
// pin the test depends on production guard order (broker check
// before onAttach check) — if the order ever flips, the assertion
// would surface as a missing-handler error and silently mis-claim
// to test the nil-broker branch.
func TestAttachSharedState_NilBrokerFailsFast(t *testing.T) {
	server := &GRPCServer{
		Impl:     &testWorkerImpl{},
		broker:   nil,
		onAttach: func(context.Context, pb.SharedStateClient) error { return nil },
	}
	_, err := server.AttachSharedState(context.Background(), &pb.AttachSharedStateRequest{BrokerId: 1})
	if err == nil {
