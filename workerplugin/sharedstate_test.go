...
func TestSharedStateStore_CacheHoldsUnderConcurrentSweepPressure(t *testing.T) {
	// Concurrent FetchAdd callers racing against a sweep goroutine
	// must never observe a torn state where the cache is present for
	// one caller but gone for the next. The
...
func TestSharedStateStore_ConcurrentSweepEvictionIsIdempotent(t *testing.T) {
	// The deleted flag and mu-guarded deletion must ensure that a
	// touchScope racing with a concurrent evicting sweep
