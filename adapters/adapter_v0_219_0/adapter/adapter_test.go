...
// BuildRequestRegistry / LegacyRegistry return the typed registry
// pointer wrapped in any. The suite uses these for identity comparison
// across calls (rejected SetClientRegistry calls must not swap the
// pointer underneath); two any values containing the same typed
// pointer compare equal with ==.
...
// ExpectedOptions is the identity transform on v0.219; the adapter
// forwards client.Options to BAML unwrapped, so the snapshot reads
// back the same map the operator supplied. v0.204 wraps every value
// through DynamicValue (the older CFFI shape requires it for nested-
// map serialisation) and overrides this hook with WrapMapValues.
