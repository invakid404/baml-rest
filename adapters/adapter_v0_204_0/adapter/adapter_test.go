...
// behaviour now that v0.204's adapter has been ported.
//
...
func TestSetClientRegistryKeepsExplicitStrategyParentForLegacyOnly(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// back to the introspected provider via
			// UpstreamClientRegistryProvider in production. The
			// expectation here is a per-row literal rather than a
			// re-invocation of the same helper so drift in
			// canonicalisation can't mask itself.
