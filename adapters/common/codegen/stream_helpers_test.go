...
func TestGenerateStreamHelpers_ContainsExpectedSymbols(t *testing.T) {
	...
	wantSubstrings := []string{
		...
		"SelectedCall",
		"startTime",
		// Verdict-15 follow-up: planned metadata must be emitted upfront
		// from the goroutine so it does not depend on BAML firing onTick.
		"emitPlanned",
		"plannedSent",
	}
	...
	// Verdict-15 follow-up structural check: emitPlanned must be invoked
	// inside both run*Orchestration goroutines BEFORE body() runs. The
	// upfront emission is the whole point — gating planned metadata on
	// onTick lost the event whenever BAML completed without firing it.
