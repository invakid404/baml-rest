	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			defer func() {
				...
				// fmt.Sprint accepts any value type — strings,
				// errors, formatted structs — so the test no longer
				// depends on the panic value being concretely a
				// string. The substring assertions below still pin
				// the message content.
				msg := fmt.Sprint(r)
