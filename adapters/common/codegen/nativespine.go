...
func EmitNativeStaticUnary(m projectdescriptor.Method, opts NativeSpineOptions) ([]byte, error) {
	...
	// Imports: fmt + bamlutils always; bytes + encoding/json only when there is an
	// output class (whose codec uses them).
	...
	// Neutral executor seam + boilerplate + registration (+ codec helpers when
	// any output class was emitted).
...
// within the M1 profile (primitive/enum/class/list/optional). Class/enum
// references use the namespaced output type name. Anything else is a fail-closed
// error.
func schemaGoType(t schemadescriptor.Type) (string, error) {
	...
	case schemadescriptor.TypePrimitive:
		...
		case schemadescriptor.PrimitiveBool:
			return "bool", nil
		default:
			return "", fmt.Errorf("primitive %q is outside the M1 profile", t.Primitive)
		}
	...
	case schemadescriptor.TypeUnion:
		...
		return "", fmt.Errorf("only optional (single nullable variant) unions are in the M1 profile")
	default:
		return "", fmt.Errorf("type kind %q is outside the M1 profile", t.Kind)
	}
}
