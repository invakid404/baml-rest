...
func init() {
	...
	for key, value := range baml_patched.Sources {
		path := filepath.Clean(fmt.Sprintf("./%s/%s", "dynclient/baml-patched", key))
		Sources[path] = value
	}
