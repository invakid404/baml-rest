...
func validateMessages(msgs []bamlutils.DynamicMessage) *Decline {
	...
	for i := range msgs {
		...
		// A message that sets BOTH a parts array and text content is ambiguous:
		// the union is one-of, and the renderer/conversion would silently prefer
		// parts and drop the text. Reject it (this also catches a non-nil but
		// empty parts slice paired with text, which the length check below misses)
		// so an ambiguous carrier falls back to BAML.
		if m.PartsContent != nil && m.TextContent != nil {
			return declinef(StageMessage, ReasonAmbiguousContent,
				"message %d carries both a parts array and text content", i)
		}
		switch {
...
func validatePart(msgIdx, partIdx int, p *bamlutils.DynamicContentPart) *Decline {
	...
	default:
		// The request-controlled part type drives nothing here and is NEVER
