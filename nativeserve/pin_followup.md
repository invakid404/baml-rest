...
`a65c511e5c53`, the branch SOURCE commit on `feat/debaml-execbridge-u1` that carries the
...
MUST carry the U1 source — which the branch commit `a65c511e5c53` does and no master commit
...
```text
STATUS: OUTSTANDING
PINNED-COMMIT: a65c511e5c53
PINNED-STAMP: 20260831153443
...
PR: feat/debaml-execbridge-u1 (this PR); pins name the branch source commit a65c511e5c53; re-pin to the master squash commit is OWED post-merge
```

## Why the pins point at the branch source commit (not master yet)

`a65c511e5c53` is the branch commit on `feat/debaml-execbridge-u1` that carries the U1
...
`nativeserve/go.mod` names `a65c511e5c53`, which is fetchable as long as the branch exists).
...
| --- | --- | --- | --- |
| 1 | `nativeserve/go.mod` | `github.com/invakid404/baml-rest` | `v0.0.0-20260831153443-a65c511e5c53` |
| 2 | `nativeserve/go.mod` | `github.com/invakid404/baml-rest/bamlutils` | `v0.0.49-0.20260831153443-a65c511e5c53` |
| 3 | `nativeserve/go.mod` | `github.com/invakid404/baml-rest/worker` | `v0.0.49-0.20260831153443-a65c511e5c53` |
| 4 | `internal/nativebody/nanollmprepare/go.mod` | `github.com/invakid404/baml-rest/bamlutils` | `v0.0.49-0.20260831153443-a65c511e5c53` |
| 5 | `internal/nativebody/nanollmprepare/go.mod` | `github.com/invakid404/baml-rest/worker` | `v0.0.49-0.20260831153443-a65c511e5c53` |
...
`a65c511e5c53` instead, and left the record `OUTSTANDING`:
...
0. **Stamp resolved by Go, off the origin — never hand-computed.** After the source commit was
   ...
   go mod download -json <mod>@a65c511e5c53` was run for the root, `bamlutils` and `worker`,
   and Go returned `v0.0.0-20260831153443-a65c511e5c53` for the root and
   `v0.0.49-0.20260831153443-a65c511e5c53` for the other two — each keeping its own base
   version. Those strings are used verbatim.
1. **All five selections re-pointed together** to `a65c511e5c53`. The edit touched only
...
**This has NOT yet been done for U1: the pins are BRANCH-ONLY** at `a65c511e5c53` and the
...
- [x] all five selections name the branch source commit `a65c511e5c53`, each with its correct
      base version
...
While the pins name the BRANCH commit `a65c511e5c53`, this file reads `STATUS: OUTSTANDING`,
