	...
	for _, r := range readers {
		...
		for sc.Scan() {
			...
			case ev.Test != "" && ev.Action == "pass":
				...
				// Subtests report as "TestX/sub"; -run slicing operates on
				// top-level funcs, so fold subtest time into its parent.
				sum.TestSeconds[ev.Package][topLevelTest(ev.Test)] += ev.Elapsed
			}
		}
...
// to named test funcs before `ingest` promotes it from count-sharding to the
...
func applyIngest(st *Store, sum *eventSummary, opt ingestOptions) *ingestReport {
	flags := canonicalFlags(opt.Race, opt.Count)
	rep := &ingestReport{Alpha: opt.Alpha, Events: sum.Events, Malformed: sum.Malformed}
