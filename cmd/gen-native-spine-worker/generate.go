...
func renderAggregate(emitted []emittedMethod) ([]byte, error) {
	...
	b.WriteString("// NewStreamOracleExecutor decodes the embedded deployment descriptor and builds\n")
	b.WriteString("// the population-filtered oracle-capable STREAM executor via\n")
	b.WriteString("// spine.NewPopulationStreamExecutor over the STREAM candidates — the SAME\n")
	b.WriteString("// candidate list and the SAME classifier NewRuntime uses, so the standard\n")
	b.WriteString("// stream population is exactly the native-only one. It ALLOWS an empty\n")
	b.WriteString("// population (an all-decline executor whose every /stream falls back to BAML),\n")
	b.WriteString("// which NewRuntime deliberately refuses. It is the ExecBridge-U1s standard\n")
	b.WriteString("// composite's construction path; this executor's own path carries no generated\n")
	b.WriteString("// BAML/CFFI — the standard worker links BAML elsewhere and injects only the\n")
	b.WriteString("// neutral no-send plan + per-prefix/final parse closures.\n")
	b.WriteString("func NewStreamOracleExecutor() (bamlutils.NativeSpineStreamOracleExecutor, error) {\n")
	b.WriteString("\tvar proj projectdescriptor.Project\n")
	b.WriteString("\tif err := json.Unmarshal(projectDescriptorJSON, &proj); err != nil {\n")
	b.WriteString("\t\treturn nil, fmt.Errorf(\"nativegenerated: decode embedded project descriptor: %w\", err)\n")
	b.WriteString("\t}\n")
	b.WriteString("\texec, err := spine.NewPopulationStreamExecutor(proj, streamCandidates(), nil)\n")
	b.WriteString("\tif err != nil {\n")
	b.WriteString("\t\treturn nil, err\n")
	b.WriteString("\t}\n")
	b.WriteString("\treturn exec, nil\n")
