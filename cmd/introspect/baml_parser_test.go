...
func TestParseClientBlock_RoundRobinStart_TrailingCloseBrace(t *testing.T) {
	...
	t.Run("quoted start rejected to match runtime", func(t *testing.T) {
		// Static introspection must agree with the runtime override
		// classifier (roundrobin/resolver.go inspectStartOverride),
		// which rejects string-shaped starts. If the introspector
		// stripped quotes before ParseInt, codegen would emit a
		// successful start for a config the runtime would reject as
		// ErrInvalidStartOverride — the support gate would
		...
		parseClientBlock(cfg, "RR", block)
		if v, ok := cfg.roundRobinStart["RR"]; ok {
			t.Fatalf(`quoted start "1" must not parse (runtime rejects string-shaped starts), got %d`, v)
		}
	})
}
...
// canonical "baml-fallback" string.
//
