...
func ensureStaticMapHelperFile(packageDir string, rewrittenSrc []byte, pkgIdx *pkgTypeIndex) error {
	...
	// `reflect` is always required: staticMapHelperCore() emits
	// bamlStaticMapCarrierIsNil (the typed-nil carrier guard) into every
	// helper file, and that guard calls reflect.ValueOf. Keep it in its
	// own stdlib group so the import block stays gofmt-clean even when no
	// baml / sibling-package imports are needed (the native-map-only
	// recursive-alias case).
...
func emitStaticMapHelper(name, typeExpr string, pkgIdx *pkgTypeIndex) string {
	...
	body.WriteString("\tif bamlStaticMapCarrierIsNil(value) {\n")
	if isPtr {
...
func emitSliceWrappedStaticMapHelper(name, typeExpr string, pkgIdx *pkgTypeIndex) string {
	...
	b.WriteString("\tif bamlStaticMapCarrierIsNil(value) {\n")
...
// staticMapHelperCore returns the supporting bits every emitted helper
// shares. It currently emits bamlStaticMapCarrierIsNil, the typed-nil
// guard the helpers call before touching the structural RangeAny/Len
// carrier interface. The helper file always includes this when at least
// one helper is emitted, and `reflect` is imported alongside it.
//
// Why the guard is required: an absent optional `map<string,T>?` field
// reaches the decode helper as an `any` whose dynamic value is a
// typed-nil `*baml.OrderedMap[T]` carrier. In Go that means:
//
//   - `value == nil` is false (the interface carries a dynamic type),
//   - the exact `*baml.OrderedMap[T]` assertion misses (the runtime
//     carrier's generic instantiation differs), but
//   - the structural `interface{ RangeAny(...); Len() int }` IS
//     satisfied, because RangeAny is a value-receiver method and a
//     typed-nil pointer still has a method set.
//
// Calling RangeAny on that nil pointer then triggers Go's
// runtime.panicwrap ("value method ... RangeAny called using nil
// pointer"), which crosses the BAML CGo callback boundary and kills the
// worker (issue #448). This mirrors the encode-side
// unwrapEncodeOrderedCarrier guard in dynamic_order_fix.go, which
// already rejects nil pointer carriers via reflect before RangeAny.
func staticMapHelperCore() string {
	return `// bamlStaticMapCarrierIsNil reports whether the decode carrier handed
