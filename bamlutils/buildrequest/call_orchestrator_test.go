...
func TestRunCallOrchestration_NilHTTPClient(t *testing.T) {
	...
	switch {
	case hasFinal:
		// Pin the single-final invariant before downstream checks:
		// RunCallOrchestration's contract emits exactly one terminal
		// final per success path. A duplicate (e.g. a producer-side
		// bug emitting both an interim and a terminal final, or two
		// terminal finals from a retry race) would silently overwrite
		// `final` and slip past the value check below; fail loudly
		// instead.
		if finalCount != 1 {
			t.Fatalf("nil-client fallback: expected exactly one terminal final per RunCallOrchestration's contract, got %d (last final=%v)", finalCount, final)
		}
		if len(streamErrs) > 0 {
	...
	case len(streamErrs) > 0:
		...
		//
		// RunCallOrchestration's contract emits exactly one terminal
		// StreamResultKindError; getting more than one would indicate
		// a producer-side bug (a duplicate emission, an error after
		// final, etc.). Pin len == 1 before the flake check so a
		// future regression that violates the single-error invariant
		// trips loudly rather than getting absorbed by the loop.
		if len(streamErrs) != 1 {
