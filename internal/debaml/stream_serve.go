...
func SupportsNativeFinalBundle(bundle *schema.Bundle) error {
	...
	// De-BAML Phase 3a/3c: classify the bundle against the narrow static-final
	// recursive-ALIAS families FIRST (before the recursive-class profile and before the
	// generic checkSupported). Each fingerprint (the exact direct five-arm `JSON` alias,
	// the exact nullable six-stored-variant `JsonValue` alias) IS a complete cut-line —
	// a value-neutral structural proof — so an admitted alias family needs no further
	// support check; every other alias bundle falls through to checkSupported's blanket
