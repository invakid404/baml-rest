...
// # How it works
//
//   - corpus_test.go enumerates 656 (expression, `this`) cases. The expression
...
func TestJinjaHelpersSourceTests(t *testing.T) {
	...
	// ACCEPTED COST, round 20. `regex_match` is withdrawn: Go's RE2 and Rust's
	// regex crate are different engines, and their Unicode classes are not proven
	// identical across the content space. BAML's own pinned case is kept here as
	// a REFUSAL so the loss stays visible rather than silently disappearing.
	if _, err := debaml.RenderConstraintExpression(phone, `this|regex_match("123")`); err == nil {
		t.Errorf("regex_match is withdrawn; expected a refusal")
	}
