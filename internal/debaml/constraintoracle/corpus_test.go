...
type constraintCase struct {
	...
	Retained string
	// Stock and Native are the pinned outcomes of the two legs. Stock != Native
	// marks a measured divergence and REQUIRES a Note.
	Stock  legOutcome
	Native legOutcome
	Note   string
}
...
var constraintCases = []constraintCase{
	...
	{Label: "op_div_zero", Group: "const", Expr: "(1 / 0) > 0", Stock: outTrue, Native: outError, Note: "engine: minijinja's `/` always produces an f64, so 1/0 is +inf and the comparison answers; minijinja-Go raises a division-by-zero error. Fail-closed."},
	{Label: "op_floordiv_zero", Group: "const", Expr: "(1 // 0) > 0", Stock: outError, Native: outError},
	{Label: "op_rem_zero", Group: "const", Expr: "(1 % 0) > 0", Stock: outError, Native: outError},
	...
	{Label: "op_str_add_num", Group: "const", Expr: "(\"a\" + 1) == \"a1\"", Stock: outError, Native: outError},
	...
	{Label: "f_split", Group: "const", Expr: "\"a,b\"|split(\",\")|length == 2", Stock: outError, Native: outTrue, Note: "engine: minijinja's `split` returns a lazy iterator with no length (`cannot calculate length of value of type iterator`); minijinja-Go's returns a materialised list."},
	...
	{Label: "f_sum_str_arg", Group: "const", Expr: "\"ab\"|sum == 0", Stock: outError, Native: outError},
	{Label: "f_sum_map", Group: "const", Expr: "{\"a\":1}|sum == 0", Stock: outError, Native: outError},
	{Label: "f_sum_attr_kwarg", Group: "const", Expr: "[{\"a\":1}]|sum(attribute=\"a\") == 1", Stock: outError, Native: outError},
	...
	{Label: "f_items", Group: "const", Expr: "{\"a\":1}|items|length == 1", Stock: outTrue, Native: outTrue},
	{Label: "f_items_order", Group: "const", Expr: "({\"z\":1,\"a\":2}|items|first|first) == \"z\"", Stock: outTrue, Native: outFalse, Note: "engine: minijinja-Go's `items` filter sorts map keys (filters.go FilterItems); minijinja preserves IndexMap insertion order under BAML's preserve_order feature."},
	...
	{Label: "f_tojson_map", Group: "const", Expr: "({\"z\":1,\"a\":2}|tojson)[2] == \"z\"", Stock: outTrue, Native: outFalse, Note: "engine: minijinja-Go's `tojson` renders a map through a Go map (sorted keys); minijinja preserves insertion order."},
	{Label: "f_urlencode", Group: "const", Expr: "\"a b\"|urlencode == \"a%20b\"", Stock: outError, Native: outError},
	...
	{Label: "f_regex_noarg", Group: "const", Expr: "\"a\"|regex_match", Stock: outError, Native: outError},
	{Label: "f_unknown", Group: "const", Expr: "1|nosuchfilter == 1", Stock: outError, Native: outError},
	...
	{Label: "t_containing", Group: "const", Expr: "\"abc\" is containing(\"b\")", Stock: outError, Native: outError},
	...
	{Label: "t_unknown", Group: "const", Expr: "1 is nosuchtest", Stock: outError, Native: outError},
	...
	{Label: "fn_cycler", Group: "const", Expr: "cycler(\"a\",\"b\").next() == \"a\"", Stock: outError, Native: outError},
	{Label: "fn_joiner", Group: "const", Expr: "joiner(\",\")() == \"\"", Stock: outError, Native: outError},
	{Label: "fn_unknown", Group: "const", Expr: "nosuchfn() == 1", Stock: outError, Native: outError},
	{Label: "py_str_upper", Group: "const", Expr: "\"abc\".upper() == \"ABC\"", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_lower", Group: "const", Expr: "\"ABC\".lower() == \"abc\"", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_startswith", Group: "const", Expr: "\"abc\".startswith(\"a\")", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_endswith", Group: "const", Expr: "\"abc\".endswith(\"c\")", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_split", Group: "const", Expr: "\"a-b\".split(\"-\")|length == 2", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_format", Group: "const", Expr: "\"{}\".format(1) == \"1\"", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_format_commas", Group: "const", Expr: "\"{:,}\".format(1234567) == \"1,234,567\"", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_format_float", Group: "const", Expr: "\"{:.2f}\".format(3.14159) == \"3.14\"", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_find", Group: "const", Expr: "\"abc\".find(\"b\") == 1", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_rfind", Group: "const", Expr: "\"abcb\".rfind(\"b\") == 3", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_count", Group: "const", Expr: "\"aba\".count(\"a\") == 2", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_strip", Group: "const", Expr: "\"  a \".strip() == \"a\"", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_lstrip", Group: "const", Expr: "\" a\".lstrip() == \"a\"", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_rstrip", Group: "const", Expr: "\"a \".rstrip() == \"a\"", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_isalpha", Group: "const", Expr: "\"abc\".isalpha()", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_isdigit", Group: "const", Expr: "\"123\".isdigit()", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_isascii", Group: "const", Expr: "\"abc\".isascii()", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_islower", Group: "const", Expr: "\"abc\".islower()", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_isupper", Group: "const", Expr: "\"ABC\".isupper()", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_isspace", Group: "const", Expr: "\"  \".isspace()", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_isalnum", Group: "const", Expr: "\"a1\".isalnum()", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_isnumeric", Group: "const", Expr: "\"1\".isnumeric()", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_title", Group: "const", Expr: "\"a b\".title() == \"A B\"", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_capitalize", Group: "const", Expr: "\"abc\".capitalize() == \"Abc\"", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_replace", Group: "const", Expr: "\"aXa\".replace(\"X\",\"-\") == \"a-a\"", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_splitlines", Group: "const", Expr: "\"a\\nb\".splitlines()|length == 2", Retained: "\"a\\\\nb\".splitlines()|length == 2", Stock: outFalse, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_str_join", Group: "const", Expr: "\",\".join([\"a\",\"b\"]) == \"a,b\"", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_seq_count", Group: "const", Expr: "[1,1].count(1) == 2", Stock: outTrue, Native: outError, Note: "engine: BAML installs minijinja-contrib's pycompat unknown-method callback; minijinja-Go v2.16.0 has no environment-level hook for it, so string and sequence methods raise `unknown callable`. Fail-closed \u2014 native errors where BAML answers, so the request declines to BAML."},
	{Label: "py_num_method", Group: "const", Expr: "(1).nosuchmethod() == 1", Stock: outError, Native: outError},
	...
	{Label: "this_int_bare", Group: "int7", Expr: "this", Stock: outError, Native: outError},
	...
	{Label: "this_int0_bare", Group: "int0", Expr: "this", Stock: outError, Native: outError},
	...
	{Label: "this_str_split", Group: "str", Expr: "this|split(\" \")|length == 2", Stock: outError, Native: outTrue, Note: "engine: same lazy-iterator `split` gap as f_split."},
	...
	{Label: "this_stre_bare", Group: "stre", Expr: "this", Stock: outError, Native: outError},
	...
	{Label: "this_map_order", Group: "map", Expr: "this|list|join(\",\") == \"z,a,m\"", Stock: outTrue, Native: outTrue},
	{Label: "this_map_in", Group: "map", Expr: "\"z\" in this", Stock: outTrue, Native: outFalse, Note: "engine: minijinja-Go evaluates `in` via value.Value.Contains, which type-switches on the concrete payload and has no object arm, so membership over the ordered mapping is always false."},
	{Label: "this_map_notin", Group: "map", Expr: "\"q\" not in this", Stock: outTrue, Native: outTrue},
	{Label: "this_map_keys", Group: "map", Expr: "this.keys()|join(\",\") == \"z,a,m\"", Stock: outTrue, Native: outTrue},
	{Label: "this_map_values", Group: "map", Expr: "this.values()|sum == 6", Stock: outTrue, Native: outTrue},
	{Label: "this_map_items", Group: "map", Expr: "this.items()|length == 3", Stock: outTrue, Native: outTrue},
	{Label: "this_map_get", Group: "map", Expr: "this.get(\"z\") == 1", Stock: outTrue, Native: outTrue},
	{Label: "this_map_get_missing", Group: "map", Expr: "this.get(\"q\") == none", Stock: outTrue, Native: outTrue},
	{Label: "this_map_get_default", Group: "map", Expr: "this.get(\"q\", 9) == 9", Stock: outTrue, Native: outTrue},
	{Label: "this_map_items_filter", Group: "map", Expr: "this|items|length == 3", Stock: outTrue, Native: outTrue},
	{Label: "this_map_dictsort", Group: "map", Expr: "(this|dictsort|first|first) == \"a\"", Stock: outTrue, Native: outTrue},
	{Label: "this_map_tojson", Group: "map", Expr: "(this|tojson)[2] == \"z\"", Stock: outTrue, Native: outFalse, Note: "engine: `tojson` reaches the ordered mapping through value.AsMap, which is an unordered Go map, so key order is lost on this seam only."},
	{Label: "this_map_ismapping", Group: "map", Expr: "this is mapping", Stock: outTrue, Native: outTrue},
	{Label: "this_map_first", Group: "map", Expr: "this|first == \"z\"", Stock: outTrue, Native: outTrue},
	{Label: "this_map_sum", Group: "map", Expr: "this|sum == 0", Stock: outError, Native: outError},
	...
	{Label: "this_cls_order", Group: "cls", Expr: "this|list|join(\",\") == \"b,a,c\"", Stock: outTrue, Native: outTrue},
	{Label: "this_cls_in", Group: "cls", Expr: "\"a\" in this", Stock: outTrue, Native: outFalse, Note: "engine: same Contains gap as this_map_in, on a class-shaped mapping."},
	...
	{Label: "this_cls_keys", Group: "cls", Expr: "this.keys()|join(\",\") == \"b,a,c\"", Stock: outTrue, Native: outTrue},
	...
	{Label: "this_null_bare", Group: "null", Expr: "this", Stock: outNoChecks, Native: outError, Note: "stock: the predicate renders `null` (BAML's none formatter), which is not a boolean \u2014 but on an OPTIONAL field the coercion swallows the failure and emits null with no checks at all instead of rejecting. Native reports the non-boolean render as an error. Fail-closed."},
	{Label: "fn_debug", Group: "const", Expr: "debug()|length > 0", Stock: outTrue, Native: outTrue},
	{Label: "fn_lipsum", Group: "const", Expr: "lipsum(1)|length > 0", Stock: outError, Native: outError},
	...
	{Label: "f_length_none", Group: "const", Expr: "none|length == 0", Stock: outError, Native: outTrue, Note: "engine: minijinja errors with `cannot calculate length of value of type none`; minijinja-Go's FilterLength returns 0 for a value with no length."},
	{Label: "f_string_list", Group: "const", Expr: "([1,2]|string)[0] == \"[\"", Stock: outTrue, Native: outTrue},
	{Label: "f_string_map", Group: "const", Expr: "({\"z\":1}|string)[0] == \"{\"", Stock: outTrue, Native: outTrue},
	{Label: "this_map_string", Group: "map", Expr: "(this|string)[0] == \"{\"", Stock: outTrue, Native: outTrue},
	{Label: "this_map_string_len", Group: "map", Expr: "(this|string)|length == 24", Stock: outTrue, Native: outTrue},
	{Label: "this_cls_string_len", Group: "cls", Expr: "(this|string)|length == 33", Stock: outFalse, Native: outFalse},
	...
	{Label: "this_map_last", Group: "map", Expr: "this|last == \"m\"", Stock: outError, Native: outTrue, Note: "engine: minijinja's `last` rejects a mapping; minijinja-Go's FilterLast iterates the object and returns its final key."},
	{Label: "this_map_values_join", Group: "map", Expr: "this.values()|join(\",\") == \"1,2,3\"", Stock: outTrue, Native: outTrue},
