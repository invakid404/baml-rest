...
var constraintCases = []constraintCase{
	...
	{Label: "op_index_neg", Group: "const", Expr: "[1,2,3][-1] == 3", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "f_replace", Group: "const", Expr: "\"aXa\"|replace(\"X\",\"-\") == \"a-a\"", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "f_format", Group: "const", Expr: "\"%s!\"|format(\"hi\") == \"hi!\"", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "f_join", Group: "const", Expr: "[1,2]|join(\"-\") == \"1-2\"", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "f_abs", Group: "const", Expr: "-1|abs == 1", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "f_items", Group: "const", Expr: "{\"a\":1}|items|length == 1", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "f_items_order", Group: "const", Expr: "({\"z\":1,\"a\":2}|items|first|first) == \"z\"", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "f_tojson_map", Group: "const", Expr: "({\"z\":1,\"a\":2}|tojson)[2] == \"z\"", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "f_regex_class", Group: "const", Expr: "\"(123)456-7890\"|regex_match(\"\\\\(?\\\\d{3}\\\\)?[-.\\\\s]?\\\\d{3}[-.\\\\s]?\\\\d{4}\")", Retained: "\"(123)456-7890\"|regex_match(\"\\\\\\\\(?\\\\\\\\d{3}\\\\\\\\)?[-.\\\\\\\\s]?\\\\\\\\d{3}[-.\\\\\\\\s]?\\\\\\\\d{4}\")", Stock: outFalse, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "this_int_arith", Group: "int7", Expr: "this + 1 == 8", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "this_map_items_filter", Group: "map", Expr: "this|items|length == 3", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "this_map_dictsort", Group: "map", Expr: "(this|dictsort|first|first) == \"a\"", Stock: outTrue, Native: outTrue},
	{Label: "this_map_tojson", Group: "map", Expr: "(this|tojson)[2] == \"z\"", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "f_string_map", Group: "const", Expr: "({\"z\":1}|string)[0] == \"{\"", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "f_maplit_order", Group: "const", Expr: "({\"z\":1,\"a\":2}|list|join(\",\")) == \"z,a\"", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "f_maplit_first", Group: "const", Expr: "{\"z\":1,\"a\":2}|first == \"z\"", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "f_maplit_string", Group: "const", Expr: "({\"z\":1,\"a\":2}|string)[2] == \"z\"", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "f_maplit_reverse", Group: "const", Expr: "({\"z\":1,\"a\":2}|reverse|first) == \"a\"", Stock: outFalse, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "f_split_first", Group: "const", Expr: "\"a,b\"|split(\",\")|first == \"a\"", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "f_split_join", Group: "const", Expr: "(\"a,b\"|split(\",\")|join(\"-\")) == \"a-b\"", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "num_exact_max", Group: "const", Expr: "9007199254740991 == 9007199254740991", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "num_conflate", Group: "const", Expr: "9007199254740993 == 9007199254740992", Stock: outFalse, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "num_at_boundary", Group: "const", Expr: "9007199254740992 == 9007199254740992", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "num_add_inrange", Group: "const", Expr: "9007199254740990 + 1 == 9007199254740991", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "num_float_big", Group: "const", Expr: "1.0e300 > 1.0e299", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "this_int_big_cmp", Group: "int7", Expr: "this * 1000000000 < 9007199254740992", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "f_int_boundary", Group: "const", Expr: "\"9007199254740993\"|int == \"9007199254740992\"|int", Stock: outFalse, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "f_int_small", Group: "const", Expr: "\"42\"|int == 42", Stock: outTrue, Native: outTrue},
	{Label: "f_int_map_boundary", Group: "const", Expr: "[\"9007199254740993\"]|map(\"int\")|first > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "f_int_float_exempt", Group: "const", Expr: "\"9007199254740993\"|float == \"9007199254740992\"|float", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "num_pow_escape", Group: "const", Expr: "2 ** 62 > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "num_mul_inrange", Group: "const", Expr: "3000000 * 3000000 == 9000000000000", Stock: outTrue, Native: outTrue},
	{Label: "this_strexact_compose", Group: "strexact", Expr: "this|int + 1 + 1 == this|int + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "this_strexact_mul", Group: "strexact", Expr: "this|int * 2 > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "this_strexact_pow", Group: "strexact", Expr: "this|int ** 2 > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "this_strexact_int", Group: "strexact", Expr: "this|int == 9007199254740991", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "this_strexact_length", Group: "strexact", Expr: "this|length == 16", Stock: outTrue, Native: outTrue},
	{Label: "this_liststr_mapsum_compose", Group: "liststr2", Expr: "this|map(\"int\")|sum + 1 + 1 == this|map(\"int\")|sum + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "this_listbig_sum_arith", Group: "listbig", Expr: "this|sum + 1 == this|sum", Stock: outFalse, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "this_listbig_sum", Group: "listbig", Expr: "this|sum > 0", Stock: outTrue, Native: outTrue},
	{Label: "this_listbig_max_arith", Group: "listbig", Expr: "this|max + 1 + 1 == this|max + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "f_length_arith", Group: "const", Expr: "[1,2]|length + 1 == 3", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "lit_hex", Group: "const", Expr: "0x20000000000001 == 0x20000000000000", Stock: outFalse, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "lit_underscore", Group: "const", Expr: "9_007_199_254_740_993 == 9_007_199_254_740_992", Stock: outFalse, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "lit_binary", Group: "const", Expr: "0b1 + 0b1 == 2", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "lit_octal", Group: "const", Expr: "0o7 + 1 == 8", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "lit_exponent", Group: "const", Expr: "1e5 + 1 == 100001", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	{Label: "ws_newline_static", Group: "const", Expr: "9007199254740991 \n + 1 \n + 1 == 9007199254740991 \n + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "fn_range_last_big", Group: "const", Expr: "range(9007199254740990.0, 9007199254740994.0, 2.0)|last == range(9007199254740992.0, 9007199254740994.0, 1.0)|last", Stock: outFalse, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
	...
	{Label: "pow_float_exp", Group: "const", Expr: "2 ** 0.5 > 1", Stock: outTrue, Native: outUnsupported, Note: "profile: integer-arithmetic CLOSURE. The expression combines a magnitude-growing operator with a filter or call, so an integer reaching that operator is not statically bounded \u2014 and a runtime integer just inside 2^53 can cross it in an operator native cannot hook. Refused by exceedsExactIntegerRange."},
