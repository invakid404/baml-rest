...
// GENERATED-ONCE CORPUS. See doc.go for what this proves and how to regenerate
// the .baml fixture from it (BAML_CONSTRAINT_FIXTURE_WRITE=1).
...
var constraintGroups = []constraintGroup{
	...
	{Name: "strnumsm", BAMLType: "string", Input: "{\"v\":\"42\"}", This: debaml.StringValue("42")},
	{Name: "strexact", BAMLType: "string", Input: "{\"v\":\"9007199254740991\"}", This: debaml.StringValue("9007199254740991")},
	{Name: "liststr2", BAMLType: "string[]", Input: "{\"v\":[\"9007199254740991\",\"1\"]}", This: debaml.ListValue([]debaml.ConstraintValue{debaml.StringValue("9007199254740991"), debaml.StringValue("1")})},
	{Name: "listbig", BAMLType: "int[]", Input: "{\"v\":[4503599627370495,4503599627370496]}", This: debaml.ListValue([]debaml.ConstraintValue{debaml.IntValue(4503599627370495), debaml.IntValue(4503599627370496)})},
}
...
var constraintCases = []constraintCase{
	...
	{Label: "f_replace", Group: "const", Expr: "\"aXa\"|replace(\"X\",\"-\") == \"a-a\"", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "f_format", Group: "const", Expr: "\"%s!\"|format(\"hi\") == \"hi!\"", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	...
	{Label: "f_join", Group: "const", Expr: "[1,2]|join(\"-\") == \"1-2\"", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	...
	{Label: "f_abs", Group: "const", Expr: "-1|abs == 1", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	...
	{Label: "f_regex_class", Group: "const", Expr: "\"(123)456-7890\"|regex_match(\"\\\\(?\\\\d{3}\\\\)?[-.\\\\s]?\\\\d{3}[-.\\\\s]?\\\\d{4}\")", Retained: "\"(123)456-7890\"|regex_match(\"\\\\\\\\(?\\\\\\\\d{3}\\\\\\\\)?[-.\\\\\\\\s]?\\\\\\\\d{3}[-.\\\\\\\\s]?\\\\\\\\d{4}\")", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	...
	{Label: "fn_range", Group: "const", Expr: "range(3)|length == 3", Stock: outTrue, Native: outUnsupported, Note: "profile: `range` is withdrawn. minijinja-Go exports no accessor for its registered globals, so range cannot be wrapped by the integer-result guard, and an out-of-range integer it produced could reach an exact comparison unchecked."},
	{Label: "fn_range_args", Group: "const", Expr: "range(1,4)|first == 1", Stock: outTrue, Native: outUnsupported, Note: "profile: `range` is withdrawn. minijinja-Go exports no accessor for its registered globals, so range cannot be wrapped by the integer-result guard, and an out-of-range integer it produced could reach an exact comparison unchecked."},
	...
	{Label: "this_int_arith", Group: "int7", Expr: "this + 1 == 8", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	...
	{Label: "f_split_join", Group: "const", Expr: "(\"a,b\"|split(\",\")|join(\"-\")) == \"a-b\"", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	...
	{Label: "num_exact_max", Group: "const", Expr: "9007199254740991 == 9007199254740991", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "num_conflate", Group: "const", Expr: "9007199254740993 == 9007199254740992", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "num_at_boundary", Group: "const", Expr: "9007199254740992 == 9007199254740992", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "num_add_inrange", Group: "const", Expr: "9007199254740990 + 1 == 9007199254740991", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	...
	{Label: "num_float_big", Group: "const", Expr: "1.0e300 > 1.0e299", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "this_int_big_cmp", Group: "int7", Expr: "this * 1000000000 < 9007199254740992", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "f_int_boundary", Group: "const", Expr: "\"9007199254740993\"|int == \"9007199254740992\"|int", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "f_int_small", Group: "const", Expr: "\"42\"|int == 42", Stock: outTrue, Native: outTrue},
	{Label: "f_int_map_boundary", Group: "const", Expr: "[\"9007199254740993\"]|map(\"int\")|first > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "f_int_float_exempt", Group: "const", Expr: "\"9007199254740993\"|float == \"9007199254740992\"|float", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "this_strnum_int", Group: "strnum", Expr: "this|int > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: runtime integer producer. The filter manufactured an integer at or past 2^53; minijinja-Go compares integers as float64, so it would be indistinguishable from its neighbour. Refused at the point of production by guardIntegerResult."},
	{Label: "this_strnum_length", Group: "strnum", Expr: "this|length == 16", Stock: outTrue, Native: outTrue},
	{Label: "this_strnumsm_int", Group: "strnumsm", Expr: "this|int == 42", Stock: outTrue, Native: outTrue},
	{Label: "num_add_escape", Group: "const", Expr: "9007199254740991 + 1 == 9007199254740992", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "num_pow_escape", Group: "const", Expr: "2 ** 62 > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "num_mul_inrange", Group: "const", Expr: "3000000 * 3000000 == 9000000000000", Stock: outTrue, Native: outTrue},
	{Label: "this_strexact_compose", Group: "strexact", Expr: "this|int + 1 + 1 == this|int + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "this_strexact_mul", Group: "strexact", Expr: "this|int * 2 > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "this_strexact_pow", Group: "strexact", Expr: "this|int ** 2 > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "this_strexact_int", Group: "strexact", Expr: "this|int == 9007199254740991", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "this_strexact_length", Group: "strexact", Expr: "this|length == 16", Stock: outTrue, Native: outTrue},
	{Label: "this_liststr_mapsum_compose", Group: "liststr2", Expr: "this|map(\"int\")|sum + 1 + 1 == this|map(\"int\")|sum + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "this_liststr_mapsum", Group: "liststr2", Expr: "this|map(\"int\")|sum > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: runtime integer producer. The filter manufactured an integer at or past 2^53; minijinja-Go compares integers as float64, so it would be indistinguishable from its neighbour. Refused at the point of production by guardIntegerResult."},
	{Label: "this_listbig_sum_arith", Group: "listbig", Expr: "this|sum + 1 == this|sum", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "this_listbig_sum", Group: "listbig", Expr: "this|sum > 0", Stock: outTrue, Native: outTrue},
	{Label: "this_listbig_max_arith", Group: "listbig", Expr: "this|max + 1 + 1 == this|max + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "f_length_arith", Group: "const", Expr: "[1,2]|length + 1 == 3", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "f_abs_arith", Group: "const", Expr: "-1|abs + 1 == 2", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "num_pow_oversized", Group: "const", Expr: "1 ** 9007199254740991 == 1", Stock: outError, Native: outUnsupported},
	{Label: "lit_hex", Group: "const", Expr: "0x20000000000001 == 0x20000000000000", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "lit_underscore", Group: "const", Expr: "9_007_199_254_740_993 == 9_007_199_254_740_992", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "lit_binary", Group: "const", Expr: "0b1 + 0b1 == 2", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "lit_octal", Group: "const", Expr: "0o7 + 1 == 8", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "lit_exponent", Group: "const", Expr: "1e5 + 1 == 100001", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "ws_newline_static", Group: "const", Expr: "9007199254740991 \n + 1 \n + 1 == 9007199254740991 \n + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "ws_newline_small", Group: "const", Expr: "1 \n + 1 == 2", Stock: outTrue, Native: outTrue},
	{Label: "this_strexact_newline", Group: "strexact", Expr: "this|int \n + 1 \n + 1 == this|int \n + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "fn_range_last_big", Group: "const", Expr: "range(9007199254740990.0, 9007199254740994.0, 2.0)|last == range(9007199254740992.0, 9007199254740994.0, 1.0)|last", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "fn_range_last_small", Group: "const", Expr: "range(3)|last == 2", Stock: outTrue, Native: outUnsupported, Note: "profile: `range` is withdrawn. minijinja-Go exports no accessor for its registered globals, so range cannot be wrapped by the integer-result guard, and an out-of-range integer it produced could reach an exact comparison unchecked."},
	{Label: "pow_negative_exp", Group: "const", Expr: "2 ** -1 == 0.5", Stock: outError, Native: outUnsupported},
	{Label: "pow_float_exp", Group: "const", Expr: "2 ** 0.5 > 1", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "pow_parenthesised", Group: "const", Expr: "2 ** (10) == 1024", Stock: outTrue, Native: outTrue},
	{Label: "pow_double_paren", Group: "const", Expr: "((2)) ** ((10)) == 1024", Stock: outTrue, Native: outTrue},
	{Label: "arith_paren_group", Group: "const", Expr: "(1 + 2) * 3 == 9", Stock: outTrue, Native: outTrue},
	{Label: "pow_computed_neg_exp", Group: "const", Expr: "2 ** (0 - 1) == 0.5", Stock: outError, Native: outUnsupported},
	{Label: "pow_computed_neg_exp2", Group: "const", Expr: "2 ** (1 - 3) == 0.25", Stock: outError, Native: outUnsupported},
	{Label: "pow_paren_neg_exp", Group: "const", Expr: "2 ** (-1) == 0.5", Stock: outError, Native: outUnsupported},
	{Label: "pow_computed_exp", Group: "const", Expr: "2 ** (1 + 1) == 4", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "pow_floordiv_exp", Group: "const", Expr: "2 ** (4 // 2) == 4", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "pow_nested", Group: "const", Expr: "2 ** 3 ** 2 == 512", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "pow_zero_exp", Group: "const", Expr: "2 ** 0 == 1", Stock: outTrue, Native: outTrue},
	{Label: "rem_negative_lhs", Group: "const", Expr: "-1 % 2 == 1", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "rem_negative_rhs", Group: "const", Expr: "7 % -3 == 1", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "rem_computed_neg", Group: "const", Expr: "(0 - 1) % 2 == 1", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "floordiv_negative_rhs", Group: "const", Expr: "1 // -2 == 0", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "floordiv_negative_lhs", Group: "const", Expr: "-7 // 2 == -4", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "floordiv_computed_neg", Group: "const", Expr: "1 // (0 - 2) == 0", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "rem_paren_literals", Group: "const", Expr: "(7) % (3) == 1", Stock: outTrue, Native: outTrue},
	{Label: "floordiv_paren_literals", Group: "const", Expr: "(7) // (2) == 3", Stock: outTrue, Native: outTrue},
	{Label: "div_negative", Group: "const", Expr: "-7 / 2 == -3.5", Stock: outTrue, Native: outTrue},
	{Label: "ws_crlf_static", Group: "const", Expr: "9007199254740991 \r\n + 1 \r\n + 1 == 9007199254740991 \r\n + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "ws_crlf_small", Group: "const", Expr: "1 \r\n + 1 == 2", Stock: outTrue, Native: outTrue},
	{Label: "this_strexact_crlf", Group: "strexact", Expr: "this|int \r\n + 1 \r\n + 1 == this|int \r\n + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "add_sub_neg_chain", Group: "const", Expr: "(1 - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999)) == (2 - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999))", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "add_sub_neg_chain_gt", Group: "const", Expr: "(1 - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999) - (-999999999999999)) > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "add_pos_chain", Group: "const", Expr: "1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 == 2 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "add_two_near_2p52", Group: "const", Expr: "4503599627370496 + 4503599627370496 == 9007199254740992", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "add_sub_neg_small", Group: "const", Expr: "1 - (-2) == 3", Stock: outTrue, Native: outTrue},
	{Label: "add_sub_chain_small", Group: "const", Expr: "7 - 2 - 1 == 4", Stock: outTrue, Native: outTrue},
	{Label: "add_pos_small", Group: "const", Expr: "1000000 + 1000000 == 2000000", Stock: outTrue, Native: outTrue},
	{Label: "add_sub_neg_big_single", Group: "const", Expr: "9007199254740991 - (-1) == 9007199254740992", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "sat_float_handoff_add", Group: "const", Expr: "(1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 - 1 + 0.0) == (1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 0.0)", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "sat_float_handoff_mul", Group: "const", Expr: "(1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 - 1) * 1.0 == (1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999) * 1.0", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "sat_float_handoff_div", Group: "const", Expr: "(1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 - 1) / 1.0 == (1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999) / 1.0", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "sat_float_handoff_sub", Group: "const", Expr: "(1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 - 1) - 0.0 == (1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999) - 0.0", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "sat_float_left", Group: "const", Expr: "0.0 + (1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 - 1) == 0.0 + (1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999)", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "sat_float_base_pow", Group: "const", Expr: "((1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999) * 1.0) ** 1 == ((1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 - 1) * 1.0) ** 1", Stock: outFalse, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "sat_float_nested", Group: "const", Expr: "(((1 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999 + 999999999999999) + 0.0) - 1.0) * 2.0 > 0.0", Stock: outTrue, Native: outUnsupported, Note: "profile: outside the proven numeric sublanguage. Numerics are admitted only with no arithmetic at all, or when the WHOLE expression parses as the closed numeric grammar whose forms are proven identical to stock across the sign and range space. This case falls outside it \u2014 an unrecognised literal form, arithmetic mixed with other syntax, an integer at or past 2^53, a signed `//`/`%`, or an exponent that is not a non-negative literal \u2014 and is refused rather than guessed at."},
	{Label: "sat_ctl_float_add", Group: "const", Expr: "(1 + 2 + 0.0) == (3 + 0.0)", Stock: outTrue, Native: outTrue},
	{Label: "sat_ctl_float_mul", Group: "const", Expr: "(1 + 2) * 1.0 == 3 * 1.0", Stock: outTrue, Native: outTrue},
	{Label: "sat_ctl_float_div", Group: "const", Expr: "(1 + 2) / 1.0 == 3.0", Stock: outTrue, Native: outTrue},
	{Label: "sat_ctl_float_left", Group: "const", Expr: "0.0 + (1 + 2) == 0.0 + 3", Stock: outTrue, Native: outTrue},
	{Label: "sat_ctl_float_base_pow", Group: "const", Expr: "(3 * 1.0) ** 1 == 3.0", Stock: outTrue, Native: outTrue},
	{Label: "sat_ctl_float_nested", Group: "const", Expr: "((1 + 2 + 0.0) - 1.0) * 2.0 > 0.0", Stock: outTrue, Native: outTrue},
}
