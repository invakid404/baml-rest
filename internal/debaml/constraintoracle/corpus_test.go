...
var constraintCases = []constraintCase{
	...
	{Label: "num_add_inrange", Group: "const", Expr: "9007199254740990 + 1 == 9007199254740991", Stock: outTrue, Native: outTrue},
	...
	{Label: "f_int_boundary", Group: "const", Expr: "\"9007199254740993\"|int == \"9007199254740992\"|int", Stock: outFalse, Native: outUnsupported, Note: "profile: runtime integer producer. The filter manufactured an integer at or past 2^53; minijinja-Go compares integers as float64, so it would be indistinguishable from its neighbour. Refused at the point of production by guardIntegerResult."},
	{Label: "f_int_small", Group: "const", Expr: "\"42\"|int == 42", Stock: outTrue, Native: outTrue},
	{Label: "f_int_map_boundary", Group: "const", Expr: "[\"9007199254740993\"]|map(\"int\")|first > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: runtime integer producer. The filter manufactured an integer at or past 2^53; minijinja-Go compares integers as float64, so it would be indistinguishable from its neighbour. Refused at the point of production by guardIntegerResult."},
	{Label: "f_int_float_exempt", Group: "const", Expr: "\"9007199254740993\"|float == \"9007199254740992\"|float", Stock: outTrue, Native: outTrue},
	...
	{Label: "num_pow_escape", Group: "const", Expr: "2 ** 62 > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	{Label: "num_mul_inrange", Group: "const", Expr: "3000000 * 3000000 == 9000000000000", Stock: outTrue, Native: outTrue},
	{Label: "this_strexact_compose", Group: "strexact", Expr: "this|int + 1 + 1 == this|int + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	{Label: "this_strexact_mul", Group: "strexact", Expr: "this|int * 2 > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	{Label: "this_strexact_pow", Group: "strexact", Expr: "this|int ** 2 > 0", Stock: outTrue, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	{Label: "this_strexact_int", Group: "strexact", Expr: "this|int == 9007199254740991", Stock: outTrue, Native: outTrue},
	{Label: "this_strexact_length", Group: "strexact", Expr: "this|length == 16", Stock: outTrue, Native: outTrue},
	{Label: "this_liststr_mapsum_compose", Group: "liststr2", Expr: "this|map(\"int\")|sum + 1 + 1 == this|map(\"int\")|sum + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	...
	{Label: "this_listbig_sum_arith", Group: "listbig", Expr: "this|sum + 1 == this|sum", Stock: outFalse, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	{Label: "this_listbig_sum", Group: "listbig", Expr: "this|sum > 0", Stock: outTrue, Native: outTrue},
	{Label: "this_listbig_max_arith", Group: "listbig", Expr: "this|max + 1 + 1 == this|max + 1", Stock: outFalse, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	{Label: "f_length_arith", Group: "const", Expr: "[1,2]|length + 1 == 3", Stock: outTrue, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
