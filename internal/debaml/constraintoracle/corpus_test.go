...
var constraintCases = []constraintCase{
	...
	{Label: "op_pow", Group: "const", Expr: "2 ** 10 == 1024", Stock: outTrue, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	...
	{Label: "op_i64max", Group: "const", Expr: "9223372036854775807 - 1 == 9223372036854775806", Stock: outTrue, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	...
	{Label: "num_exact_max", Group: "const", Expr: "9007199254740991 == 9007199254740991", Stock: outTrue, Native: outTrue},
	{Label: "num_conflate", Group: "const", Expr: "9007199254740993 == 9007199254740992", Stock: outFalse, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	{Label: "num_at_boundary", Group: "const", Expr: "9007199254740992 == 9007199254740992", Stock: outTrue, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	{Label: "num_add_escape", Group: "const", Expr: "9007199254740990 + 1 == 9007199254740991", Stock: outTrue, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	{Label: "num_i64_min", Group: "const", Expr: "0 - 9223372036854775807 == -9223372036854775807", Stock: outTrue, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	{Label: "num_mul_escape", Group: "const", Expr: "3037000500 * 3037000500 > 9007199254740992", Stock: outTrue, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
	{Label: "num_small_mul", Group: "const", Expr: "1000 * 1000 == 1000000", Stock: outTrue, Native: outTrue},
	{Label: "num_pow_small", Group: "const", Expr: "2 ** 3 == 8", Stock: outTrue, Native: outTrue},
	{Label: "num_float_big", Group: "const", Expr: "1.0e300 > 1.0e299", Stock: outTrue, Native: outTrue},
	{Label: "this_int_big_cmp", Group: "int7", Expr: "this * 1000000000 < 9007199254740992", Stock: outTrue, Native: outUnsupported, Note: "profile: 64-bit numeric boundary. minijinja-Go computes and compares integers as float64, so at or past 2^53 the answer is wrong, and past 2^63 the int64 conversion is architecture-dependent (arm64 saturates, amd64 does not). Refused by the static magnitude bound in exceedsExactIntegerRange."},
}
