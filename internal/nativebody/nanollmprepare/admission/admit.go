...
// built (and header-preflighted) but never sent, and the secret-free
// alias/target/provider. The nanollm engine that produced the plan has already
// been Closed inside Admit (the no-send path never needs it to outlive the
// call), so an Admitted holds no engine, no client, and no credential.
