...
// nilProneExecutor is a minimal neutral STREAM executor whose POINTER methods deref the
// receiver, so a typed-nil (*nilProneExecutor)(nil) boxed in the interface would panic on
// the first dispatch — exactly the footgun NewNativeRuntime must reject at CONSTRUCTION.
...
func (e *nilProneExecutor) Stream(context.Context, string, any, bamlutils.NativeSpineStreamEmit) bamlutils.NativeSpineStreamResult {
	_ = e.marker
	return bamlutils.SucceededSpineStreamResult(nil, "", "")
}

func (e *nilProneExecutor) ParseStream(context.Context, string, string) (any, error) {
	_ = e.marker
	return nil, nil
}

var _ bamlutils.NativeSpineStreamExecutor = (*nilProneExecutor)(nil)
...
func TestNewNativeRuntimeRejectsNilExecutor(t *testing.T) {
	assertPanics := func(name string, exec bamlutils.NativeSpineStreamExecutor) {
		defer func() {
			if recover() == nil {
