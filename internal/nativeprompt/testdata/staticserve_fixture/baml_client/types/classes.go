...
type StaticFloatCheckedAnswer struct {
	Answer     string           `json:"answer"`
	Confidence Checked[float64] `json:"confidence"`
}
...
func (c *StaticFloatCheckedAnswer) Decode(holder *cffi.CFFIValueClass, typeMap baml.TypeMap) {
	...
	for _, field := range holder.Fields {
		...
		case "confidence":
			c.Confidence = FromStockChecked(baml.Decode(valueHolder).Interface().(StockChecked[float64]))
...
type StaticUnionCheckedAnswer struct {
	Answer     string                     `json:"answer"`
	Confidence Checked[Union2IntOrString] `json:"confidence"`
}
...
func (c *StaticUnionCheckedAnswer) Decode(holder *cffi.CFFIValueClass, typeMap baml.TypeMap) {
	...
	for _, field := range holder.Fields {
		...
		case "confidence":
			c.Confidence = FromStockChecked(baml.Decode(valueHolder).Interface().(StockChecked[Union2IntOrString]))
