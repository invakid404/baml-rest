...
import (
	...
	bamlclient "github.com/invakid404/baml-rest/internal/nativeprompt/testdata/staticserve_op_fixtures/le/baml_client"
	streamtypes "github.com/invakid404/baml-rest/internal/nativeprompt/testdata/staticserve_op_fixtures/le/baml_client/stream_types"
	types "github.com/invakid404/baml-rest/internal/nativeprompt/testdata/staticserve_op_fixtures/le/baml_client/types"
	adapter "github.com/invakid404/baml-rest/internal/nativeprompt/testdata/staticserve_op_fixtures/le/generated/adapter"
	introspected "github.com/invakid404/baml-rest/internal/nativeprompt/testdata/staticserve_op_fixtures/le/introspected"
	"slices"
	"strings"
	"sync"
	"sync/atomic"
...
type StaticAssertConfidenceInput struct {
	Topic string `json:"topic"`
}
type StaticAssertConfidenceOutput struct {
	kind         bamlutils.StreamResultKind
	raw          string
	reasoning    string
	streamParsed *streamtypes.StaticAssertAnswer
	finalParsed  *types.StaticAssertAnswer
	err          error
	reset        bool
	metadata     *bamlutils.Metadata
}

func (v *StaticAssertConfidenceOutput) Kind() bamlutils.StreamResultKind {
	return v.kind
}
func (v *StaticAssertConfidenceOutput) Stream() any {
	return v.streamParsed
}
func (v *StaticAssertConfidenceOutput) Final() any {
	return v.finalParsed
}
func (v *StaticAssertConfidenceOutput) Error() error {
	return v.err
}
func (v *StaticAssertConfidenceOutput) Raw() string {
	return v.raw
}
func (v *StaticAssertConfidenceOutput) Reasoning() string {
	return v.reasoning
}
func (v *StaticAssertConfidenceOutput) Reset() bool {
	return v.reset
}
func (v *StaticAssertConfidenceOutput) Metadata() *bamlutils.Metadata {
	return v.metadata
}

var staticAssertConfidenceOutputPool = bamlutils.NewPool(func() *StaticAssertConfidenceOutput {
	return &StaticAssertConfidenceOutput{}
})

func (v *StaticAssertConfidenceOutput) Release() {
	if v == nil {
		return
	}
	*v = StaticAssertConfidenceOutput{}
	staticAssertConfidenceOutputPool.Put(v)
}
func getStaticAssertConfidenceOutput() *StaticAssertConfidenceOutput {
	return staticAssertConfidenceOutputPool.Get()
}
func newStaticAssertConfidenceOutputError(err error) *StaticAssertConfidenceOutput {
	r := getStaticAssertConfidenceOutput()
	r.kind = bamlutils.StreamResultKindError
	r.err = err
	return r
}
func newStaticAssertConfidenceOutputMetadata(md *bamlutils.Metadata) *StaticAssertConfidenceOutput {
	r := getStaticAssertConfidenceOutput()
	r.kind = bamlutils.StreamResultKindMetadata
	r.metadata = md
	return r
}
func staticAssertConfidenceNoRaw(adapter bamlutils.Adapter, rawInput any, out chan bamlutils.StreamResult, skipPartials bool, plannedMetadata *bamlutils.Metadata, clientOverride string) error {
	options, err := makeLegacyStreamOptionsFromAdapter(adapter, clientOverride)
	if err != nil {
		return err
	}
	input, ok := rawInput.(*StaticAssertConfidenceInput)
	if !ok {
		return fmt.Errorf("invalid input type: expected *%s, got %T", "StaticAssertConfidenceInput", rawInput)
	}
	return runNoRawOrchestration(adapter, out, func() bamlutils.StreamResult {
		__r := getStaticAssertConfidenceOutput()
		__r.kind = bamlutils.StreamResultKindHeartbeat
		return __r
	}, func(err error) bamlutils.StreamResult {
		return newStaticAssertConfidenceOutputError(err)
		...
		stream, streamErr := bamlclient.Stream.StaticAssertConfidence(adapter, input.Topic, streamOpts...)
		if streamErr != nil {
			__errR := newStaticAssertConfidenceOutputError(streamErr)
			select {
			case out <- __errR:
			case <-adapter.Done():
				__errR.Release()
		...
		for streamVal := range stream {
			...
			if streamVal.IsError {
				__errR := newStaticAssertConfidenceOutputError(streamVal.Error)
				select {
				case out <- __errR:
				case <-adapter.Done():
					__errR.Release()
...
func staticAssertConfidenceFull(adapter bamlutils.Adapter, rawInput any, out chan bamlutils.StreamResult, skipIntermediateParsing bool, plannedMetadata *bamlutils.Metadata, clientOverride string) error {
	options, err := makeLegacyStreamOptionsFromAdapter(adapter, clientOverride)
	if err != nil {
		return err
	}
	...
	return runFullOrchestration(adapter, out, options, func() bamlutils.StreamResult {
		...
		__r := newStaticAssertConfidenceOutputError(err)
		__r.raw = raw
		return __r
	}, func(__r bamlutils.StreamResult) {
		__r.Release()
		...
		parsed, parseErr := bamlclient.ParseStream.StaticAssertConfidence(adapter, parseable, options...)
		if parseErr == nil {
			select {
			case <-adapter.Done():
				return nil
		...
		stream, streamErr := bamlclient.Stream.StaticAssertConfidence(adapter, input.Topic, driveOpts...)
		if streamErr != nil {
		...
		if result != nil {
			if ptr, ok := result.(*types.StaticAssertAnswer); ok {
				__r.finalParsed = ptr
			} else if val, ok := result.(types.StaticAssertAnswer); ok {
				__r.finalParsed = &val
			}
		}
		return __r
...
func staticAssertConfidenceBuildRequest(adapter bamlutils.Adapter, rawInput any, out chan bamlutils.StreamResult, provider string, retryPolicy *retry.Policy, fallbackChain []string, clientProviders map[string]string, legacyChildren map[string]bool, fallbackTargets map[string]string, fallbackRoundRobin map[string]*bamlutils.RoundRobinInfo, plannedMetadata *bamlutils.Metadata, clientOverride string) error {
	...
	buildRequestFn := func(ctx context.Context, clientOverride string) (*llmhttp.Request, error) {
		...
		httpReq, err := bamlclient.StreamRequest.StaticAssertConfidence(ctx, input.Topic, callOpts...)
		if err != nil {
			return nil, err
		}
	...
	buildBedrockStreamRequestFn := func(ctx context.Context, clientOverride string) (*llmhttp.Request, error) {
		...
		httpReq, err := bamlclient.Request.StaticAssertConfidence(ctx, input.Topic, callOpts...)
		if err != nil {
			return nil, err
		}
		url, urlErr := httpReq.Url()
		...
		if selectedClient == "" {
			selectedClient = introspected.FunctionClient["StaticAssertConfidence"]
		}
		var (
	...
	parseStreamFn := func(ctx context.Context, accumulated string) (any, error) {
		return bamlclient.ParseStream.StaticAssertConfidence(ctx, accumulated, options...)
	}
	parseFinalFn := func(ctx context.Context, accumulated string) (any, error) {
		return bamlclient.Parse.StaticAssertConfidence(ctx, accumulated, options...)
	}
	newResultFn := func(kind bamlutils.StreamResultKind, stream any, final any, raw string, reasoning string, err error, reset bool) bamlutils.StreamResult {
		...
		r.reset = reset
		if stream != nil {
			if v, ok := stream.(*streamtypes.StaticAssertAnswer); ok {
				r.streamParsed = v
			} else if v, ok := stream.(streamtypes.StaticAssertAnswer); ok {
				r.streamParsed = &v
			}
		}
		if final != nil {
			if v, ok := final.(*types.StaticAssertAnswer); ok {
				r.finalParsed = v
			} else if v, ok := final.(types.StaticAssertAnswer); ok {
				r.finalParsed = &v
			}
		}
		return r
	...
	legacyStreamChildFn := func(ctx context.Context, clientOverride string, _ string, needsRaw bool, sendHeartbeat func()) (any, string, string, error) {
		...
		return runLegacyChildStream(ctx, needsRaw, sendHeartbeat, func(onTick func(context.Context, pkg.TickReason, pkg.FunctionLog) pkg.FunctionSignal) (any, error) {
			...
			stream, streamErr := bamlclient.Stream.StaticAssertConfidence(ctx, input.Topic, opts...)
			if streamErr != nil {
				return nil, streamErr
			}
			var result any
...
func staticAssertConfidenceBuildCallRequest(adapter bamlutils.Adapter, rawInput any, out chan bamlutils.StreamResult, provider string, retryPolicy *retry.Policy, fallbackChain []string, clientProviders map[string]string, legacyChildren map[string]bool, fallbackTargets map[string]string, fallbackRoundRobin map[string]*bamlutils.RoundRobinInfo, plannedMetadata *bamlutils.Metadata, clientOverride string) error {
	options, err := makeOptionsFromAdapter(adapter)
	if err != nil {
		return err
	}
	...
	parseFinalFn := func(ctx context.Context, text string) (any, error) {
		return bamlclient.Parse.StaticAssertConfidence(ctx, text, options...)
	}
	newResultFn := func(kind bamlutils.StreamResultKind, stream any, final any, raw string, reasoning string, err error, reset bool) bamlutils.StreamResult {
		r := getStaticAssertConfidenceOutput()
		r.kind = kind
	...
	if __staticServe != nil || __staticShadow != nil {
		if __staticDescriptor, __staticOK := introspected.StaticPromptDescriptor("StaticAssertConfidence"); __staticOK {
			if __staticValues, __staticValuesOK := introspected.StaticPromptArgumentValues("StaticAssertConfidence", []any{input.Topic}); __staticValuesOK {
				if __staticServe != nil {
					installNativeStaticCall(callConfig, __staticServe, adapter, __staticDescriptor, map[string]any{"topic": input.Topic}, []string{"topic"}, __staticValues, len(fallbackChain) == 0, len(fallbackChain) > 0, plannedMetadata != nil && plannedMetadata.RoundRobin != nil, retryPolicy != nil, adapter.StreamMode().NeedsRaw(), func(__pctx context.Context, __raw string) ([]byte, error) {
						__pr, __pe := bamlclient.Parse.StaticAssertConfidence(__pctx, __raw, options...)
						if __pe != nil {
							return nil, __pe
						}
...
func StaticAssertConfidence(adapter bamlutils.Adapter, rawInput any) (<-chan bamlutils.StreamResult, error) {
	out := make(chan bamlutils.StreamResult, 100)
	var err error
	mode := adapter.StreamMode()
	__retryClient := buildrequest.ResolvePrimaryClient(adapter, introspected.FunctionClient["StaticAssertConfidence"])
	__effective := __retryClient
	var __rrInfo *bamlutils.RoundRobinInfo
	__rrEffective, __rrInfoUpgrade, __rrErr := buildrequest.ResolveEffectiveClient(adapter, introspected.FunctionClient["StaticAssertConfidence"], introspected.FallbackChains, introspected.ClientProvider, introspected.RoundRobinCoordinator)
	if __rrErr != nil {
		return nil, __rrErr
	}
	...
	if introspected.Request != nil && (mode == bamlutils.StreamModeCall || mode == bamlutils.StreamModeCallWithRaw) {
		...
		if provider != "" && buildrequest.IsCallProviderSupported(provider) {
			...
			err = staticAssertConfidenceBuildCallRequest(adapter, rawInput, out, provider, retryPolicy, nil, nil, nil, nil, nil, __planned, __effective)
			if err != nil {
				return nil, err
			}
			return out, nil
	...
	if introspected.StreamRequest != nil && (mode == bamlutils.StreamModeStream || mode == bamlutils.StreamModeStreamWithRaw) {
		...
		if provider != "" && buildrequest.IsProviderSupported(provider) {
			...
			__planned.RoundRobin = __rrInfo
			err = staticAssertConfidenceBuildRequest(adapter, rawInput, out, provider, retryPolicy, nil, nil, nil, nil, nil, __planned, __effective)
			if err != nil {
				return nil, err
			}
			return out, nil
		...
		if __resolution != nil && len(__resolution.Chain) > 0 {
			...
			__planned.RoundRobin = __rrInfo
			err = staticAssertConfidenceBuildRequest(adapter, rawInput, out, "", retryPolicy, __resolution.Chain, __resolution.Providers, __resolution.LegacyChildren, __resolution.Targets, __resolution.NestedRoundRobin, __planned, "")
			if err != nil {
				return nil, err
			}
			return out, nil
	...
	if introspected.StreamRequest != nil && (mode == bamlutils.StreamModeCall || mode == bamlutils.StreamModeCallWithRaw) {
		...
		if __resolution != nil && len(__resolution.Chain) > 0 {
			...
			if __callChainSupported {
				...
				__planned.RoundRobin = __rrInfo
				err = staticAssertConfidenceBuildCallRequest(adapter, rawInput, out, "", retryPolicy, __resolution.Chain, __resolution.Providers, __resolution.LegacyChildren, __resolution.Targets, __resolution.NestedRoundRobin, __planned, "")
				if err != nil {
					return nil, err
				}
				return out, nil
	...
	buildrequest.LogLegacyClassification(adapter, "StaticAssertConfidence", __plannedLegacy)
	switch mode {
	case bamlutils.StreamModeCall:
		err = staticAssertConfidenceNoRaw(adapter, rawInput, out, true, __plannedLegacy, __legacyClientOverride)
	case bamlutils.StreamModeStream:
		err = staticAssertConfidenceNoRaw(adapter, rawInput, out, false, __plannedLegacy, __legacyClientOverride)
	case bamlutils.StreamModeCallWithRaw:
		err = staticAssertConfidenceFull(adapter, rawInput, out, true, __plannedLegacy, __legacyClientOverride)
	case bamlutils.StreamModeStreamWithRaw:
		err = staticAssertConfidenceFull(adapter, rawInput, out, false, __plannedLegacy, __legacyClientOverride)
	default:
...
type StaticCheckedConfidenceOutput struct {
	kind         bamlutils.StreamResultKind
	raw          string
	reasoning    string
	streamParsed *streamtypes.StaticCheckedAnswer
	finalParsed  *types.StaticCheckedAnswer
	err          error
	reset        bool
	metadata     *bamlutils.Metadata
}

func (v *StaticCheckedConfidenceOutput) Kind() bamlutils.StreamResultKind {
	return v.kind
}
func (v *StaticCheckedConfidenceOutput) Stream() any {
	return v.streamParsed
}
func (v *StaticCheckedConfidenceOutput) Final() any {
	return v.finalParsed
}
func (v *StaticCheckedConfidenceOutput) Error() error {
	return v.err
}
func (v *StaticCheckedConfidenceOutput) Raw() string {
	return v.raw
}
func (v *StaticCheckedConfidenceOutput) Reasoning() string {
	return v.reasoning
}
func (v *StaticCheckedConfidenceOutput) Reset() bool {
	return v.reset
}
func (v *StaticCheckedConfidenceOutput) Metadata() *bamlutils.Metadata {
	return v.metadata
}

var staticCheckedConfidenceOutputPool = bamlutils.NewPool(func() *StaticCheckedConfidenceOutput {
	return &StaticCheckedConfidenceOutput{}
})

func (v *StaticCheckedConfidenceOutput) Release() {
	if v == nil {
		return
	}
	*v = StaticCheckedConfidenceOutput{}
	staticCheckedConfidenceOutputPool.Put(v)
}
func getStaticCheckedConfidenceOutput() *StaticCheckedConfidenceOutput {
	return staticCheckedConfidenceOutputPool.Get()
}
func newStaticCheckedConfidenceOutputError(err error) *StaticCheckedConfidenceOutput {
	r := getStaticCheckedConfidenceOutput()
	r.kind = bamlutils.StreamResultKindError
	r.err = err
	return r
}
func newStaticCheckedConfidenceOutputMetadata(md *bamlutils.Metadata) *StaticCheckedConfidenceOutput {
	r := getStaticCheckedConfidenceOutput()
	r.kind = bamlutils.StreamResultKindMetadata
	r.metadata = md
	return r
}
func staticCheckedConfidenceNoRaw(adapter bamlutils.Adapter, rawInput any, out chan bamlutils.StreamResult, skipPartials bool, plannedMetadata *bamlutils.Metadata, clientOverride string) error {
	options, err := makeLegacyStreamOptionsFromAdapter(adapter, clientOverride)
	if err != nil {
		return err
	}
	input, ok := rawInput.(*StaticCheckedConfidenceInput)
	if !ok {
		return fmt.Errorf("invalid input type: expected *%s, got %T", "StaticCheckedConfidenceInput", rawInput)
	}
	return runNoRawOrchestration(adapter, out, func() bamlutils.StreamResult {
		__r := getStaticCheckedConfidenceOutput()
		__r.kind = bamlutils.StreamResultKindHeartbeat
		return __r
	}, func(err error) bamlutils.StreamResult {
		return newStaticCheckedConfidenceOutputError(err)
	}, func(__r bamlutils.StreamResult) {
		__r.Release()
	}, plannedMetadata, func(md *bamlutils.Metadata) bamlutils.StreamResult {
		...
		stream, streamErr := bamlclient.Stream.StaticCheckedConfidence(adapter, input.Topic, streamOpts...)
		if streamErr != nil {
			__errR := newStaticCheckedConfidenceOutputError(streamErr)
			select {
			case out <- __errR:
			case <-adapter.Done():
				__errR.Release()
		...
		for streamVal := range stream {
			...
			if streamVal.IsError {
				__errR := newStaticCheckedConfidenceOutputError(streamVal.Error)
				select {
				case out <- __errR:
				case <-adapter.Done():
					__errR.Release()
...
func staticCheckedConfidenceFull(adapter bamlutils.Adapter, rawInput any, out chan bamlutils.StreamResult, skipIntermediateParsing bool, plannedMetadata *bamlutils.Metadata, clientOverride string) error {
	options, err := makeLegacyStreamOptionsFromAdapter(adapter, clientOverride)
	if err != nil {
		return err
	}
	...
	return runFullOrchestration(adapter, out, options, func() bamlutils.StreamResult {
		...
		__r := newStaticCheckedConfidenceOutputError(err)
		__r.raw = raw
		return __r
	}, func(__r bamlutils.StreamResult) {
		__r.Release()
		...
		parsed, parseErr := bamlclient.ParseStream.StaticCheckedConfidence(adapter, parseable, options...)
		if parseErr == nil {
			select {
			case <-adapter.Done():
				return nil
		...
		stream, streamErr := bamlclient.Stream.StaticCheckedConfidence(adapter, input.Topic, driveOpts...)
		if streamErr != nil {
			return nil, streamErr
		}
		var result any
		...
		__r.reasoning = reasoning
		if result != nil {
			if ptr, ok := result.(*types.StaticCheckedAnswer); ok {
				__r.finalParsed = ptr
			} else if val, ok := result.(types.StaticCheckedAnswer); ok {
				__r.finalParsed = &val
			}
		}
		return __r
...
func staticCheckedConfidenceBuildRequest(adapter bamlutils.Adapter, rawInput any, out chan bamlutils.StreamResult, provider string, retryPolicy *retry.Policy, fallbackChain []string, clientProviders map[string]string, legacyChildren map[string]bool, fallbackTargets map[string]string, fallbackRoundRobin map[string]*bamlutils.RoundRobinInfo, plannedMetadata *bamlutils.Metadata, clientOverride string) error {
	options, err := makeOptionsFromAdapter(adapter)
	if err != nil {
		return err
	}
	...
	buildRequestFn := func(ctx context.Context, clientOverride string) (*llmhttp.Request, error) {
		...
		httpReq, err := bamlclient.StreamRequest.StaticCheckedConfidence(ctx, input.Topic, callOpts...)
		if err != nil {
			return nil, err
		}
		url, urlErr := httpReq.Url()
	...
	buildBedrockStreamRequestFn := func(ctx context.Context, clientOverride string) (*llmhttp.Request, error) {
		...
		httpReq, err := bamlclient.Request.StaticCheckedConfidence(ctx, input.Topic, callOpts...)
		if err != nil {
			return nil, err
		}
		url, urlErr := httpReq.Url()
		...
		selectedClient := clientOverride
		if selectedClient == "" {
			selectedClient = introspected.FunctionClient["StaticCheckedConfidence"]
		}
		var (
			bedrockEndpointURL        string
			bedrockEndpointURLPresent bool
	...
	parseStreamFn := func(ctx context.Context, accumulated string) (any, error) {
		return bamlclient.ParseStream.StaticCheckedConfidence(ctx, accumulated, options...)
	}
	parseFinalFn := func(ctx context.Context, accumulated string) (any, error) {
		return bamlclient.Parse.StaticCheckedConfidence(ctx, accumulated, options...)
	}
	newResultFn := func(kind bamlutils.StreamResultKind, stream any, final any, raw string, reasoning string, err error, reset bool) bamlutils.StreamResult {
		r := getStaticCheckedConfidenceOutput()
		r.kind = kind
		...
		r.reset = reset
		if stream != nil {
			if v, ok := stream.(*streamtypes.StaticCheckedAnswer); ok {
				r.streamParsed = v
			} else if v, ok := stream.(streamtypes.StaticCheckedAnswer); ok {
				r.streamParsed = &v
			}
		}
		if final != nil {
			if v, ok := final.(*types.StaticCheckedAnswer); ok {
				r.finalParsed = v
			} else if v, ok := final.(types.StaticCheckedAnswer); ok {
				r.finalParsed = &v
			}
		}
		return r
	...
	legacyStreamChildFn := func(ctx context.Context, clientOverride string, _ string, needsRaw bool, sendHeartbeat func()) (any, string, string, error) {
		...
		return runLegacyChildStream(ctx, needsRaw, sendHeartbeat, func(onTick func(context.Context, pkg.TickReason, pkg.FunctionLog) pkg.FunctionSignal) (any, error) {
			opts := append(callOpts, bamlclient.WithOnTick(onTick))
			stream, streamErr := bamlclient.Stream.StaticCheckedConfidence(ctx, input.Topic, opts...)
			if streamErr != nil {
				return nil, streamErr
			}
			var result any
...
func staticCheckedConfidenceBuildCallRequest(adapter bamlutils.Adapter, rawInput any, out chan bamlutils.StreamResult, provider string, retryPolicy *retry.Policy, fallbackChain []string, clientProviders map[string]string, legacyChildren map[string]bool, fallbackTargets map[string]string, fallbackRoundRobin map[string]*bamlutils.RoundRobinInfo, plannedMetadata *bamlutils.Metadata, clientOverride string) error {
	options, err := makeOptionsFromAdapter(adapter)
	if err != nil {
		return err
	}
	...
	parseFinalFn := func(ctx context.Context, text string) (any, error) {
		return bamlclient.Parse.StaticCheckedConfidence(ctx, text, options...)
	}
	newResultFn := func(kind bamlutils.StreamResultKind, stream any, final any, raw string, reasoning string, err error, reset bool) bamlutils.StreamResult {
		r := getStaticCheckedConfidenceOutput()
		r.kind = kind
	...
	__staticShadow := deBAMLStaticShadow(adapter)
	if __staticServe != nil || __staticShadow != nil {
		if __staticDescriptor, __staticOK := introspected.StaticPromptDescriptor("StaticCheckedConfidence"); __staticOK {
			if __staticValues, __staticValuesOK := introspected.StaticPromptArgumentValues("StaticCheckedConfidence", []any{input.Topic}); __staticValuesOK {
				if __staticServe != nil {
					installNativeStaticCall(callConfig, __staticServe, adapter, __staticDescriptor, map[string]any{"topic": input.Topic}, []string{"topic"}, __staticValues, len(fallbackChain) == 0, len(fallbackChain) > 0, plannedMetadata != nil && plannedMetadata.RoundRobin != nil, retryPolicy != nil, adapter.StreamMode().NeedsRaw(), func(__pctx context.Context, __raw string) ([]byte, error) {
						__pr, __pe := bamlclient.Parse.StaticCheckedConfidence(__pctx, __raw, options...)
						if __pe != nil {
							return nil, __pe
						}
						return json.Marshal(__pr)
...
func StaticCheckedConfidence(adapter bamlutils.Adapter, rawInput any) (<-chan bamlutils.StreamResult, error) {
	out := make(chan bamlutils.StreamResult, 100)
	var err error
	mode := adapter.StreamMode()
	__retryClient := buildrequest.ResolvePrimaryClient(adapter, introspected.FunctionClient["StaticCheckedConfidence"])
	__effective := __retryClient
	var __rrInfo *bamlutils.RoundRobinInfo
	__rrEffective, __rrInfoUpgrade, __rrErr := buildrequest.ResolveEffectiveClient(adapter, introspected.FunctionClient["StaticCheckedConfidence"], introspected.FallbackChains, introspected.ClientProvider, introspected.RoundRobinCoordinator)
	if __rrErr != nil {
		return nil, __rrErr
	}
	__effective = __rrEffective
	...
	if introspected.Request != nil && (mode == bamlutils.StreamModeCall || mode == bamlutils.StreamModeCallWithRaw) {
		...
		if provider != "" && buildrequest.IsCallProviderSupported(provider) {
			...
			__planned := buildrequest.BuildSingleProviderPlanForClient(__effective, provider, retryPolicy, buildrequest.BuildRequestAPIRequest)
			__planned.RoundRobin = __rrInfo
			err = staticCheckedConfidenceBuildCallRequest(adapter, rawInput, out, provider, retryPolicy, nil, nil, nil, nil, nil, __planned, __effective)
			if err != nil {
				return nil, err
			}
			return out, nil
	...
	if introspected.StreamRequest != nil && (mode == bamlutils.StreamModeStream || mode == bamlutils.StreamModeStreamWithRaw) {
		...
		if provider != "" && buildrequest.IsProviderSupported(provider) {
			...
			__planned := buildrequest.BuildSingleProviderPlanForClient(__effective, provider, retryPolicy, buildrequest.BuildRequestAPIStreamRequest)
			__planned.RoundRobin = __rrInfo
			err = staticCheckedConfidenceBuildRequest(adapter, rawInput, out, provider, retryPolicy, nil, nil, nil, nil, nil, __planned, __effective)
			if err != nil {
				return nil, err
			}
			return out, nil
		...
		if __resolution != nil && len(__resolution.Chain) > 0 {
			...
			__planned := buildrequest.BuildFallbackChainPlanFromResolution(__effective, __resolution, retryPolicy, buildrequest.BuildRequestAPIStreamRequest)
			__planned.RoundRobin = __rrInfo
			err = staticCheckedConfidenceBuildRequest(adapter, rawInput, out, "", retryPolicy, __resolution.Chain, __resolution.Providers, __resolution.LegacyChildren, __resolution.Targets, __resolution.NestedRoundRobin, __planned, "")
			if err != nil {
				return nil, err
			}
			return out, nil
	...
	if introspected.StreamRequest != nil && (mode == bamlutils.StreamModeCall || mode == bamlutils.StreamModeCallWithRaw) {
		...
		if __resolution != nil && len(__resolution.Chain) > 0 {
			...
			if __callChainSupported {
				__planned := buildrequest.BuildFallbackChainPlanFromResolution(__effective, __resolution, retryPolicy, buildrequest.BuildRequestAPIRequest)
				__planned.RoundRobin = __rrInfo
				err = staticCheckedConfidenceBuildCallRequest(adapter, rawInput, out, "", retryPolicy, __resolution.Chain, __resolution.Providers, __resolution.LegacyChildren, __resolution.Targets, __resolution.NestedRoundRobin, __planned, "")
				if err != nil {
					return nil, err
				}
				return out, nil
	...
	__plannedLegacy.RoundRobin = __rrInfo
	__legacyClientOverride := __effective
	buildrequest.LogLegacyClassification(adapter, "StaticCheckedConfidence", __plannedLegacy)
	switch mode {
	case bamlutils.StreamModeCall:
		err = staticCheckedConfidenceNoRaw(adapter, rawInput, out, true, __plannedLegacy, __legacyClientOverride)
	case bamlutils.StreamModeStream:
		err = staticCheckedConfidenceNoRaw(adapter, rawInput, out, false, __plannedLegacy, __legacyClientOverride)
	case bamlutils.StreamModeCallWithRaw:
...
var Methods = map[string]bamlutils.StreamingMethod{
	"StaticAssertConfidence": {
		Impl: StaticAssertConfidence,
		MakeInput: func() any {
			return new(StaticAssertConfidenceInput)
		},
		MakeOutput: func() any {
			return new(types.StaticAssertAnswer)
		},
	...
	"StaticCheckedConfidence": {
		...
		MakeInput: func() any {
			return new(StaticCheckedConfidenceInput)
		},
		MakeOutput: func() any {
...
func parseStaticAssertConfidence(adapter bamlutils.Adapter, raw string) (any, error) {
	options, err := makeOptionsFromAdapter(adapter)
	if err != nil {
		return nil, err
	}
	result, parseErr := bamlclient.Parse.StaticAssertConfidence(adapter, raw, options...)
	if parseErr != nil {
		return nil, parseErr
	}
	return result, nil
}
func parseStaticAssertConfidenceStream(adapter bamlutils.Adapter, raw string) (any, error) {
	options, err := makeOptionsFromAdapter(adapter)
	if err != nil {
		return nil, err
	}
	result, parseErr := bamlclient.ParseStream.StaticAssertConfidence(adapter, raw, options...)
	if parseErr != nil {
		return nil, parseErr
	}
	return result, nil
...
func parseStaticCheckedConfidence(adapter bamlutils.Adapter, raw string) (any, error) {
	...
	result, parseErr := bamlclient.Parse.StaticCheckedConfidence(adapter, raw, options...)
	if parseErr != nil {
		return nil, parseErr
	}
	return result, nil
}
func parseStaticCheckedConfidenceStream(adapter bamlutils.Adapter, raw string) (any, error) {
	options, err := makeOptionsFromAdapter(adapter)
	if err != nil {
		return nil, err
	}
...
var ParseMethods = map[string]bamlutils.ParseMethod{
	"StaticAssertConfidence": {
		Impl: parseStaticAssertConfidence,
		MakeOutput: func() any {
			return new(types.StaticAssertAnswer)
		},
